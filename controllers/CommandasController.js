...
exports.getCommandeforGiornataeCompetenzaestato = (req,res)=> {
    ...
    if(comp  == 1) {
        if(stato === 2) {
            strsql =  strSql + ' where `commandas`.`idGiornata` = ' + id + " and `commandas`.`numProdottiCucina` > 0  and `commandas`.`stato` < 3 ";
        } else {
            // per stati 3 e 4
            strsql =  strSql + ' where `commandas`.`idGiornata` = ' + id + " and `commandas`.`numProdottiCucina` > 0  and `commandas`.`stato` = " + stato + sort;
        }
    }
    if(comp  == 2) {
        strsql =  strSql + ' where `commandas`.`idGiornata` = ' + id + " and `commandas`.`numProdottiBevande` > 0  and `commandas`.`stato` = " + stato + sort;
    }
