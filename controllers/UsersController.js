...
const strSql = 'select `users`.`id`, `cognome`, `nome`, `idStato`, `tipoacc`, `username`, `password`, `email`, `idRuolo`, `idRuolo_Day`, `idruoloweb`, `noteutente`, `photo`, `eseguitaAbilitazione`, `remember_token`, `email_verified_at`, `key_utenti_operation`, `created_at`, `updated_at`' +
                ' FROM `users` ' 
...
exports.getUserbyid = (req,res)=> {
    ...
    'where `users`.`id` = ' + key;
    */
    const strsql = strSql + ' where `users`.`id` = ' + key;
   ...
   // let strsql = `select * from users where id= ${key} `;    originale
    ...
    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'2 errore il lettura users for key ' + key);

            res.status(500).send({
                message: `2 errore il lettura users for key ${key}- errore: ${err}`,
                data:null
            });
            return;
        }
...
exports.getUserbylevel = (req,res)=> {
    ...
    'where `users`.`id` = ' + key;
    */
    const strsql = strSql + ' where `users`.`idRuolo_Day` = ' + key;
   ...
   // let strsql = `select * from users where id= ${key} `;    originale
    ...
    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'errore il lettura users for livello - key ' + key);

            res.status(500).send({
                message: `errore il lettura users for livello -  key ${key}- errore: ${err}`,
                data:null
            });
            return;
        }
        ...
        if(result.length>0) {
            ...
            res.status(200).send({ 
                message:`situazione attuale per utente livello: .....  ${key}`,
                number:  result.length,
                data:result
            });                    
        }else {
            console.log(`nessun record presente per il livello: ${key} `);
            res.status(200).send({
                message: `nessun user pressente for il livello: ${key}`,
                number:  result.length,
                data:null
            });
        }
...
exports.getUserbyemail = (req,res)=> {
   ...
   // let strsql = `select * from users where email= '${key}' `;

   const strsql = strSql + " where `email` = '" + key + "' ";

    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'errore il lettura users for email ' + key);

            res.status(500).send({
                message: `errore il lettura users for email ${key}- errore: ${err}`,
                data:null
            });
            return;
        }
        ...
        if(result.length>0) {
            ...
            res.status(200).send({ 
                message:`situazione attuale per utente email: .....  ${key}`,
                data:result[0]
            });                    
        }else {
...
exports.getUserbytipo = (req,res)=> {
    ...
    let key = req.params.idruolo;
    let strsql = '';
    if(key == 0) {
        strsql =  strSql + ' where `idruoloweb` = ' + key;            //`select * from users where idruoloweb = 0 `;
    } else {
    ...
    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'errore il lettura users for idruoloweb ' + key);

            res.status(500).send({
                message: `errore il lettura users for ruoloweb ${key}- errore: ${err}`,
                data:null
            });
            return;
        }
...
exports.createNewUser = (req,res)=> {
  ...
  */
      ...
      let strsql =  `insert into users
                  (cognome,nome,photo,idStato,tipoacc,username,password,email,idRuolo,idRuolo_Day,idruoloweb,noteUtente,key_utenti_operation) 
                  valueS
                  (
                     '${cognome}','${nome}','${photo}','${idStato}','${tipoacc}','${username}','${password}','${email}','${idRuolo}','${idRuolo_Day}','${idruoloweb}','${noteUtente}','${key_utenti_operation}' 
                  )`;
...
exports.deleteUser = (req,res)=> {  
    ...
    console.log(req.body,`cancellazione utente id ${key}`);  // visualizzo la struttura dei campi immessi dall'utente 

    // definisco la strsql per lettura utente

    let strsql_Inqu = `select * from users where id= ${key} `;

    let strsql =  `delete from users  where id = ${key}`;
    ...
    // verifico prima l'esistenza del record
     ...
     db.query(strsql_Inqu,(err,result)=> {  
        if(err) {
            res.status(500).send({
                message: `errore in lettura users for key ${key} - errore: ${err}`,
                data:null
            });
            return;
        }
        if(result.length>0) {
                db.query(strsql,(err,result) => {    
                    if(err) { 
                        console.log(err,`----- errore in cancellazkione utente id: ${key}`);
                        res.status(500).send({ 
                    ...
                    res.status(200).send({ 
                        message: `Utente  id: ${key} cancellato regolarmente  `,
                        rc: 'ok',
                        data:null
                    }); 
                 });  
                    ...
                    console.log(`----- inesistente utente id: ${key} -- cancellazione non possibile`);
                    res.status(200).send({ 
                        message: `nessun user pressente for id: ${key}  -- cancellazione non possibile  `,
                        rc: 'nf',
                        data:null
                    });                     
                    return;
