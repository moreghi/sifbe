...
exports.getAllRuoli = (req,res)=> {
    ...
    let strsql = 'select * from t_ruolodays';
    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'errore il lettura all t_ruolodays');
        }
        if(result.length>0) {
...
exports.getRuolobyid = (req,res)=> {
    ...
    let strsql = `select * from t_ruolodays where id= ${id} `;

    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'errore il lettura t_ruolodays for id ' + id);       }
...
exports.createNewRuolo = (req,res)=> {
  ...
  */
      ...
      let strsql =  `insert into t_ruolodays
                  (id,d_ruolo_day,tappo,key_utenti_operation) 
                  valueS
                  (
                    ${id},UPPER('${d_ruolo_day}'),'${tappo}',${key_utenti_operation} 
                  )`;
      
    
      db.query(strsql,(err,result) => {
          if(err) {
              console.log(err,'errore in registrazione nuovo ruolo su tabella t_ruolodays ');
          }
  ...
  exports.updateRuoloByid = (req,res)=> {  
    ...
    let strsql_Inqu = `select * from t_ruolodays where id= ${id} `;
    ...
    let strsql =  `update t_ruolodays set
                    d_ruolo_day = UPPER'${d_ruolo_day}'),
                    tappo = '${tappo}',
...
exports.updateRuoloByid1 = (req,res)=> {
 ...
 db.query(strsql_Inqu,(err,result)=> {  
        ...
        if(result.length>0) {
                  db.query('UPDATE t_ruolodays SET ? WHERE id = ' + req.params.id, ruolonew,(err,result) => {    
                    if(err) { 
                        console.log(err,`----- errore in aggiornamento ruolo id: ${id}`);
                        req.flash('error', err);
                        return;
...
exports.deleteRuolo = (req,res)=> {  
    ...
    let strsql =  `delete from t_ruolodays  where id = ${id}`;
