...
//  per criptare e decriptare password
// https://devnews.it/posts/605725856637133f0f8fca12/come-criptare-decriptare-javascript-nodejs

/*    codice per criptare/decrptare
...
/*   attenzione: articoli molto importanti

https://bcrypt.online/
https://bcrypt-online.translate.goog/?_x_tr_sl=en&_x_tr_tl=it&_x_tr_hl=it&_x_tr_pto=sc

Hashing delle password con Node.js e bcrypt
https://www-abeautifulsite-net.translate.goog/posts/hashing-passwords-with-nodejs-and-bcrypt?_x_tr_sl=en&_x_tr_tl=it&_x_tr_hl=it&_x_tr_pto=sc
...
// originaria  fino al 31/12/2023
/*
const strSql = 'select `users`.*, `t_stato_utentes`.`d_stato_utente`' +
                ' FROM `users` ' + 
                ' JOIN `t_stato_utentes` ON `t_stato_utentes`.`id` = `users`.`idStato` '  */

const strSql =  'select `users`.*, `t_stato_utentes`.`d_stato_utente`, `userlevels`.`id` as idLevel, `userlevels`.`userlevelname` as dRuolo ' +
                ' FROM `users` ' + 
                ' JOIN `t_stato_utentes` ON `t_stato_utentes`.`id` = `users`.`idStato` ' +
		            ' JOIN `userlevels` ON `userlevels`.`id` = `users`.`idlevel` '  
...
console.log('authController ---- sono fuori')


exports.login = (req, res) => {


    console.log('backend--login ' + JSON.stringify(req.body));
    ...
    db.query(strsql,(err,result)=> {
        ...
        if(result.length>0) {
            ...
            var passwordIsValid = bcrypt.compareSync(
              ...
              if (!passwordIsValid) {
                return res.status(200).send({    // era impostato 401
                  accessToken: null,
                  rc: 'ko',
...
// non esiste il file di richiesta cambio password
/*
          ChangePass.destroy({
            where: {
             email: req.body.email
  ...
  exports.forgotpassword = (req,res) => {
    ...
    let token = Math.random().toString(36).substring(8, 14)   //      crypto.randomBytes(5).toString('hex') ;    //randomcodprenString();


    console.log('`forgotpassword ---- parametri passatmi--  email : ' + JSON.stringify(req.body) ); 
    ...
    let strsql1 =  "update users set token = '" + token + "' where `users`.`email` = '" + email + "' ";
    ...
    console.log('backend - forgot password - strsql --> ' + strsql);
    db.query(strsql,(err,result)=> {
        if(err) {
           res.status(500).send({
                message: `3 errore il lettura all users - erro: ${err}`,
        ...
        if(result.length>0) {
          ...
          console.log('dopo aver letto user per email ' + JSON.stringify(result[0]))
          ...
          db.query(strsql1,(err,result)=> {
            ...
            console.log('lettura utente ----- invio email ' + result.length);
                send_gmmailfor_forgotPassword(sendto,cognome,nome,token)  
                res.status(200).send({
                    rc: 'ok',
                    message: 'inviata correttamente email'
                });
              } else {
                res.status(200).send({ 
                  message: `email inesistente - impossibile inviare richietsa reset password `,
                  rc: 'nf',
                  data:null
                 }) 
              }
...
*/
 ...
 async function send_gmmailfor_forgotPassword(sendto,cognome,nome,token) {
  let message;
  ...
  const confURL = `${urlfrontend}/forgotpasswordConferme?email=${sendto}&token=${token}`;



// originario  fino al 2023/12/29
   //   const confURL = `http://localhost:4200/forgotpasswordConferme?email=${sendto}`;
      message = `<p>Buongiorno sig ${cognome} ${nome}</p>
                  ...
                  <br>
                  <p>devi inserire il codice qui a fianco riportato per attivare la funzione di reset password</p>
                  <br>
                  <p>Codice personale di richiesta   <strong>${token}</strong></p>
                  <br>
...
/*
  ...
  // 2023/06/20  effettuo la lettura ultimo record inserito e verifico password
  exports.createUserbyRegister = async (req,res)=> {

    const strsql1 = 'SELECT * FROM `users`  WHERE `users`.`id` <=99999 order by `users`.`id` desc';
    ...
    console.log(`password da body da fare hash: ${req.body.password}`);
    const hash = bcrypt.hashSync(req.body.password, 10);  // ok  salt
...
// effettuo la lettura ultimo record inserito e controllo la password criptata
            db.query(strsql1,(err,result) => {
              if(err) {
                  console.log(err,'errore in lettura ultimo user inserito su tabella users ');
                  res.status(500).send({
                    message: `errore in lettura ultimo user inserito su tabella users - errore: ${err}`,
                    rc: 'kk',
                    data:null
                });
                return;
              ...
              if(result.length>0) {
                console.log('verifico la password criptata con quella che crito ora ' + req.body.password);
                console.log('password in mysql ' + result[0].password);
  // verifico la password criptata


              if (bcrypt.compareSync(req.body.password, result[0].password)) {
                console.log('password esatta  ........------------------------- . ');
              } else {
                console.log('password errata ......... ');
                  ...
                  if(hash !== result[0].password) {
                      ...
                      res.status(200).send({
                      message: `user inserita regolarmente -- password verificata `,
                      rc: 'ok',
                      pwdc: hash,
                      passworduser: result[0].password,
                      data: result[0]
...
// metodo di registrazione utente originale senza lettura dopo insert e verifica password 2023/06/20
exports.createUserbyRegisterOriginale = async (req,res)=> {
  ...
  const hash1 = bcrypt.hash(req.body.password,salt);   // moreno 2023/06/19


  console.log(` effettuato hash da body ----> ${hash}`);

  console.log(` effettuato hash da body --      utilizzato  hash --> ${hash1}`);  // moreno 2023/06/19
// moreno 20/06/2023
...
if(passwordIsValid) {
console.log('verificata password con ------> hashSync')
}
...
if(passwordIsValid) {
console.log('verificata password con -------------------- hash normale')
}
