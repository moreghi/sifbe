...
const strSql = 'select listinos.* ' +  
               ' from  listinos  '; 
            
const order =  ' order by listinos.id desc';
...
exports.getAll = (req,res)=> {
     ...
     db.query(strsql,(err,result)=> {
        if(err) {
           res.status(500).send({
                message: `3xss errore il lettura all listinos - erro: ${err}`,
                data:null
            });
            return;
        }
...
exports.getbyid = (req,res)=> {
    ...
    let strsql = strSql + ' where `listinos`.`id` = ' + id;
   ...
   // let strsql = `select * from users where id= ${id} `;    originale
    ...
    db.query(strsql,(err,result)=> {
        if(err) {
            console.log(err,'2fgtyh errore lettura listinos for id ' + id);

            res.status(500).send({
                message: `2vvcv errore  lettura listinos for id ${id}- errore: ${err}`,
                data:null
            });
            return;
        }
...
exports.createNew = (req,res)=> {
    ...
    const strsql1 ="SELECT * FROM `listinos` WHERE id < 99999 ORDER BY id DESC;";
     ...
     let key_utenti_operation = req.body.key_utenti_operation;
       ...
       let strsql =  `insert into listinos
                  (descrizione,stato,key_utenti_operation) 
                  valueS
                  (
                    UPPER('${descrizione}'),${stato}, ${key_utenti_operation} 
                  )`;
     ...
     db.query(strsql,(err,result) => {
         if(err) {
            console.log(err,'errore in registrazione nuova listino su tabella listinos ');
            res.status(500).send({
              message: `errore in registrazione nuova listino su tabella listinos - errore: ${err}`,
              data:null
           });
           return;
         }
         ...
         db.query(strsql1,(err,result) => {
           if(err) {
                        console.log(err,'errore in lettura ultima manigfestazione  su tabella listinos ');
                        res.status(500).send({
                        message: `errore in lettura ultima listino su tabella listinos - errore: ${err}`,
                        rc: 'kk',
                        data:null
             });
             return;
  ...
  exports.updateByid = (req,res)=> {  
    ...
    let strsql_Inqu = `select * from listinos where id= ${id} `;

    let descrizione = req.body.descrizione;
    let stato = req.body.stato;
    let key_utenti_operation = req.body.key_utenti_operation;
     
    let strsql =  `update listinos set
    ...
    // verifico prima l'esistenza del record
     ...
     db.query(strsql_Inqu,(err,result)=> {  
        if(err) {
            console.log(err,'4 errore il lettura listinos for id ' + id);
            res.status(500).send({
                message: `4 errore il lettura listinos for id ${id} - errore: ${err}`,
                data:null
            });
            return;
        }
...
exports.updateByid1 = (req,res)=> {
 ...
 db.query(strsql_Inqu,(err,result)=> {  
        ...
        if(result.length>0) {
                  db.query('UPDATE listinos SET ? WHERE id = ' + req.params.id, manifw,(err,result) => {    
                    if(err) { 
...
exports.delete = (req,res)=> {  
    ...
    let strsql =  `delete from listinos  where id = ${id}`;
 
     db.query(strsql_Inqu,(err,result)=> {  
        if(err) {
            res.status(500).send({
                message: `errore in lettura listinos for id ${id} - errore: ${err}`,
                data:null
            });
            return;
        }
